...
public class CoinPrison(string? filePath, Dictionary<OutPoint, PrisonedCoinRecord> bannedCoins) : IDisposable
{
	...
	protected readonly Dictionary<OutPoint, PrisonedCoinRecord> _bannedCoins = bannedCoins;

	public void Ban(SmartCoin coin, DateTimeOffset until)
	{
		lock (_lock)
		{
			if (_bannedCoins.ContainsKey(coin.Outpoint) || until < DateTimeOffset.UtcNow)
			{
				return;
			}

			var effectiveBanningTime = EffectiveBanningTime(until);
			_bannedCoins.Add(coin.Outpoint, new PrisonedCoinRecord(coin.Outpoint, effectiveBanningTime));
			ToFile();
		}
	}
	...
	{
		return !_bannedCoins.TryGetValue(outpoint, out var bannedCoin)
	...
	protected virtual void ToFile()
	{
		...
		string json = JsonConvert.SerializeObject(_bannedCoins.Values, Formatting.Indented);
